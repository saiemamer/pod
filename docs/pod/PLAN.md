...
- `pnpm typecheck:pod` (`config/pod-typecheck-changed.mjs`) typechecks only the changed files and their imports. The full `pnpm typecheck:web` needs `NODE_OPTIONS=--max-old-space-size=6144` on this 8 GB Mac (four minutes cold, twenty seconds warm); Node's default 2 GB heap dies even with the incremental cache. CI runs the full check either way.
...
On main as `62cc0740ae` (`pod(phase3): lineage graph, sqlglot column lineage, React Flow canvas, Database explorer, orca dbt column-lineage`). Built in this order: shared types and pure graph walks with tests, the catalog reader and graph service, the sqlglot sidecar and column-lineage engine, IPC, RPC and CLI, then the canvas, the Database panel and the Playwright smoke.
...
Saiem compared the Lineage tab with dbt-zed's canvas and asked for a consistent style before Phase 4; no restyle has been made yet. Differences on record: dbt-zed colours the node border by materialisation and glows the selected node, draws column-to-column edges as smooth curves between rows, shows a materialisation legend at the toolbar's right, and keeps one toolbar row (Upstream, Downstream, Columns, Arrange, zoom). Pod tints the header, uses a ring for the focus node, draws column edges between handles, has no legend, and stacks the dock's tab strip and the lineage toolbar as two bars of different heights with the model name repeated on both. The Phase 2 dock tab strip itself (Table, Compiled, Lineage, Connection) reads as misaligned next to the toolbar. Saiem approved the proposal the same evening and asked to see it live before it lands. The restyle is built on branch `pod/phase3-lineage-restyle` (Pod-owned files only): materialisation colours as Pod tokens in `lineage-theme.css` (rose source, violet view, emerald table, amber incremental, sky seed and snapshot, slate ephemeral; lighter shades under `.dark`), the node border and header wash in that colour with a glow on the focused model, a legend of the kinds present at the toolbar's right, the dock's tab strip as underlined text on the same grid as the toolbar, zoom controls with the percentage in the toolbar (React Flow's floating controls removed), the canvas opening at 100 % centred on the focused model (fit-all stays a button), and dagre's network-simplex ranker instead of longest-path so two consumers of one model sit stacked in the same column. Verified with `ui-lineage-smoke.mjs` (now also a fan-out step through a second consumer, `orders_by_customer`, in the stand-in manifest) in light and dark mode. Saiem's first live review (2026-09-08, late evening) found three defects, fixed on the branch: the tab underline inherited a 5 px offset from the primitive and sat below the divider (now on the divider, spanning the text; the override needs the primitive's own variant prefix or tailwind-merge keeps both), a hovered column row painted past the rounded bottom corners (the row list now clips to a 5 px radius), and the top corners drew a different radius from the bottom (same fix), plus node edges now meet the box at the title row. A second look at 250 % zoom showed the row hover stopping short of the border with a seam and the corners still unequal: the row list's `overflow-hidden` had put it on its own compositing layer, and the inner radii were 5 px inside an 8 px border. Now the last row itself is rounded to `calc(var(--radius-md) - 1px)`, the header uses the same inner radius, the list has no padding, and the hover is a foreground mix because `--accent` is nearly white in the light theme. The tab list fills the header row and its underline covers the divider under the text; the toolbar's left padding matches the first tab's text. Round three: the dock's run label was pinned to the top of the header once the tab list stretched (the header centres its items again, the tab list stretches alone) and is hidden on the Lineage tab, and the Database panel reserves the hover action's width on every row so the right-hand column aligns, with an 8 px inset. Round four, motion: the dock's tab underline is one shared element that slides to the active tab (`use-pod-dbt-dock-motion.ts`, transform and width over 200 ms on the app's `cubic-bezier(0.16, 1, 0.3, 1)`), and the body flies in from the side of the chosen tab (Web Animations API, 220 ms, 12 px), following the "fly rather than teleport" and directional rules Saiem pointed at in benji.org/family-values; both stand still under `prefers-reduced-motion`. Round five, the Lineage tab's blink and the principles across Pod's surfaces: the Lineage view was unmounted on every tab change and rebuilt on return (spinner, nodes at raw positions, then the jump to the focus). It now stays mounted once opened (hidden, not removed), its chunk is warmed when the dock first appears, and the canvas holds opacity 0 until React Flow reports the nodes measured and the 100 % fit on the focus has run, then fades in already in place (`data-ready` on `pod-lineage-canvas`); a frame-by-frame sample after clicking back shows the viewport transform unchanged and every node present. Layout changes (collapse, expand, depth) tween node positions in JavaScript on the same curve (`use-tweened-positions.ts`), so edges travel with their nodes; new nodes and highlight edges fade in (opacity only, because React Flow positions nodes with an inline transform). The dock rises into place when it first appears and its height eases on collapse and expand (not during a drag); the upstream/downstream list slides in; explorer rows fade in and the hover action eases; column rows ease their colour. Everything is inert under `prefers-reduced-motion`. Upstream Orca surfaces were left alone on purpose. Performance gate (2026-09-08, Saiem's condition for the merge): `docs/pod/smoke/perf-fixture.mjs`, `graph-bench.mjs` and `ui-lineage-perf.mjs` (see the smoke README). Node numbers on the 1,005-node fixture: manifest parse 11 ms, catalog parse 16 ms, graph build with inheritance 7 ms, neighbourhood 0.6 ms, dagre for 93 nodes with columns 67 ms, cold sqlglot over 93 models 2.1 s, warm 8 ms. The in-app run failed 11 of 18 checks on 2026-09-08 late (first open 2.8 s, zoom 22.5 ms avg with a 450 ms spike at the columns threshold, column click 0.94 s, depth change 1.9 s, long tasks up to 1.2 s, panel filter 308 ms; the rest green, memory flat); the fixes so far stopped a measurement loop that had made a column click take 51 s, virtualised the Database panel and cut idle handles. Gate passed on 2026-09-08 (19 checks, dev build under React StrictMode, 8 GB Intel Mac): graph IPC 70 ms; first open 568 ms to a ready canvas of 93 nodes with a 159 ms longest task; zoom frames 18.6 ms average and 21.2 ms p95, pan 17.2 and 19.4; column lineage 65 ms cold and 16 ms warm; click to lit path 264 ms with a 101 ms task; depth change 660 ms with a 263 ms task; 114 ms longest task across twenty tab round trips and ten clicks; heap +0.7 MB after GC; Database panel open 376 ms, filter 176 ms with a 107 ms task. What it took, in the order the profiles pointed:

- Layout: `layoutLineage` spread dagre's label after computing the top-left corner, so dagre's centre overwrote it and every box sat half its height too low; tall boxes overlapped the next one by half the height difference minus the gap, which is the overlap seen on the fixture. Boxes now sit where dagre put them, 48 px apart in a column (`LINEAGE_NODE_SEP`), and the layout snapshot pins the gap. The "columns matched by name" footer is not part of the sizing (that re-laid out the canvas on every click); the gap absorbs its 19 px, and it shows only on nodes of the lit path.
- Identity: node data and React Flow node and edge objects are cached per id and rebuilt only when a field changed, so React Flow and the memoised node skip them; `dimmed` moved to a wrapper class, the focus column reaches only the focus node, the click callback reads the current answer through a ref, the highlight and the name-matched set are memoised in the hook, the requested depth is its own state (echoing the answer's depth re-fetched and rebuilt every node once per open), and a merge keeps the base node objects.
- Rows: column rows exist only for nodes within a screen of the viewport (`use-lineage-rows-window.ts`: a same-height stand-in elsewhere, three nodes per frame as they come near, dropped two screens away); below the zoom cut they are hidden by a stylesheet attribute rather than unmounted. Lit rows and footers read a small external store (`lineage-highlight-store.ts`), so a click re-renders about fifty rows and no node; nodes off the path and edges off the path fade through one canvas attribute; handles of lit rows are measured a frame after they mount (`useUpdateNodeInternals`), with React Flow's "handle not found" warning silenced for that frame.
- First open: the lineage chunk is fetched when a `.sql` editor opens (1.7 s cold on the dev build); the nodes around the focus mount first, the fit runs on the focus alone, and the rest stream sixteen per frame nearest-first (`use-lineage-first-mount.ts`). The gate now waits for the count to settle and asserts the whole neighbourhood arrived.
- Dock: the Lineage view stays laid out behind the other tabs and is hidden by opacity under an always-mounted cover, because `display`, `visibility`, `inert` and inserting a sibling each restyle the canvas's fifteen hundred elements (26 to 130 ms measured); the last non-lineage body stays mounted, so a round trip allocates little and GC pauses stopped landing in the loop. The view and the toolbar are memoised; the zoom label subscribes to the viewport on its own.
- Explorer: a native title instead of a Radix tooltip per row, a plain button for the hover action, overscan of six rows.
- Toolbar: the zoom buttons step by ten percent through `zoomTo` (pinch and wheel stay continuous).
- Gate script: `POD_PERF_PROFILE=1` writes a CPU profile per timed step; the filter check waits for the filtered rows instead of two frames.
...
Second release defect, found when the tap bump for 0.1.10 got a 404: electron-builder publishes the x64 and arm64 targets in parallel, each found no release for the tag and created one, so every tag since 0.1.8 carried two GitHub releases with the DMGs on whichever was created second; the `releases/download/<tag>/` URL resolves to the first, which held one blockmap for 0.1.10. The stub releases were deleted (tags untouched), and `pod-release.yml` now creates one draft release before packaging (electron-builder's GitHub publisher returns an existing draft for the tag and attaches to it), verifies both built apps (update feed, bundled CLI version, and a twenty-second launch of each) while the release is still a draft, then checks that exactly one release carries the tag and publishes it; the tap's bump job only sees a release once it is published. The next tag is the first to run that path. Still deferred by decision: code signing, so a DMG installed by hand rather than through Homebrew meets Gatekeeper's unverified-developer prompt (the cask's postflight clears it for brew installs).
