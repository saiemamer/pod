...
3. The tap [saiemamer/homebrew-pod](https://github.com/saiemamer/homebrew-pod) picks the release up within 30 minutes (its `bump.yml` workflow; run it by hand to skip the wait) and users get it with `brew upgrade --cask pod` followed by `xattr -dr com.apple.quarantine /Applications/Pod.app` (Homebrew 6 dropped `--no-quarantine`). Running apps also show the release in the update card.
