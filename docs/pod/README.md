...
1. Tag `v0.x.y` on `main` and push the tag. `package.json` keeps upstream's version; the workflow stamps Pod's from the tag, which is what keeps rebases free of version conflicts.
2. `Pod release (macOS, unsigned)` builds on a `macos-15` runner and publishes the DMGs plus `latest-mac.yml` to the tag's GitHub release, then checks that the packaged app updates from Pod and that its bundled CLI starts.
3. The tap [saiemamer/homebrew-pod](https://github.com/saiemamer/homebrew-pod) picks the release up within 30 minutes (its `bump.yml` workflow; run it by hand to skip the wait) and users get it with `brew upgrade --cask pod` or the "Update with Homebrew" button on the in-app card (the cask's postflight clears the quarantine attribute, since Homebrew 6 dropped `--no-quarantine`). Running apps also show the release in the update card.

Signing is what turns the update card into a working in-place update: Electron's autoUpdater refuses to install an unsigned app on macOS ("Your application must be signed for automatic updates on macOS. This is a requirement of Squirrel.Mac."). It needs an Apple Developer Program membership and a Developer ID Application certificate. Then add `MAC_CERTS`, `MAC_CERTS_PASSWORD`, `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, `APPLE_TEAM_ID` as repository secrets, set `ORCA_MAC_RELEASE=1` on the package step, and drop `CSC_IDENTITY_AUTO_DISCOVERY`.
